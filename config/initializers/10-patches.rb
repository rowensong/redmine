...
module Propshaft
  ...
  Helper.prepend(Module.new do
    def compute_asset_path(path, options = {})
      # Serve theme assets statically from /themes to avoid Propshaft processing in development
      if path.to_s.start_with?("themes/")
        return path.start_with?("/") ? path : "/#{path}"
      end
      super
    rescue MissingAssetError => e
      # Fallback to static path (avoid forcing Propshaft /assets prefix for theme files)
